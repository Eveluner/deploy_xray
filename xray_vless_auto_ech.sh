...
# ECH 配置（可选）
# 如果需要启用 ECH，请提供 ECH 密钥文件，格式为 base64 编码的 ECH 配置
USE_ECH="false"
ECH_KEY_FILE=""  # 留空则不启用 ECH
...
CONFIGEOF
  ...
  echo "  USE_ECH       - 是否启用 ECH（可选，默认 false）"
  echo "  ECH_KEY_FILE  - ECH 密钥文件路径（可选）"
  echo ""
}

echo "========== Xray VLESS 自动部署脚本（ECH 支持版） =========="
...
# ECH 密钥处理
ECH_CONFIG=""
if [ "$USE_ECH" = "true" ] && [ -n "$ECH_KEY_FILE" ]; then
  if [ ! -f "$ECH_KEY_FILE" ]; then
    echo "[WARN] ECH 密钥文件不存在: $ECH_KEY_FILE，将跳过 ECH 配置"
    USE_ECH="false"
  else
    # 读取 ECH 密钥文件内容（假设为 base64 编码）
    ECH_CONTENT=$(cat "$ECH_KEY_FILE" | tr -d '\n')
    ECH_CONFIG="\"echOutlines\": [\"$ECH_CONTENT\"],"
    echo "[OK] ECH 配置已加载"
  fi
fi
...
if [ "$USE_ECH" = "true" ]; then
  echo "    ECH 支持: 已启用"
fi
echo ""

# 生成 TLS 配置（根据是否启用 ECH）
if [ "$USE_ECH" = "true" ] && [ -n "$ECH_CONFIG" ]; then
  TLS_CONFIG=$(cat << EOF
        "tlsSettings": {
          "serverName": "$DOMAIN",
          "certificates": [
...
[Unit]
Description=Xray Service (ECH Enabled)
...
域名: $DOMAIN
端口: $PORT
ECH 支持: $([ "$USE_ECH" = "true" ] && echo "已启用" || echo "未启用")
...
update_info() {
  ...
  # 获取 ECH 支持状态
  ECH_STATUS=$(grep "^ECH 支持:" "$INFO_FILE" 2>/dev/null | awk -F": " '{print $2}' || echo "未知")
