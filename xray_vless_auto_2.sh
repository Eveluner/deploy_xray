...
stop_xray() {
  echo "==> 停止 Xray 服务"
  systemctl stop xray 2>/dev/null || { echo "[WARN] Xray 服务停止失败或未运行"; return 1; }
  echo "[OK] Xray 已停止"
}
...
start_xray() {
  echo "==> 启动 Xray 服务"
  systemctl start xray 2>/dev/null || { echo "[ERR] Xray 服务启动失败"; return 1; }
  echo "[OK] Xray 已启动"
}
...
restart_xray() {
  echo "==> 重启 Xray 服务"
  systemctl restart xray 2>/dev/null || { echo "[ERR] Xray 服务重启失败"; return 1; }
  echo "[OK] Xray 已重启"
}
...
install_xray_from_zip() {
  ...
  if [ -z "$zip_path" ] || [ ! -f "$zip_path" ]; then
    echo "[ERR] Xray 压缩包不存在: $zip_path"
    return 1
  fi

  echo "==> 解压 Xray 到 $XRAY_DIR"
  mkdir -p "$XRAY_DIR"
  if ! unzip -o "$zip_path" -d "$XRAY_DIR" >/dev/null 2>&1; then
    echo "[ERR] 解压失败"
    return 1
  fi
  chmod +x "$XRAY_DIR/xray"
...
update_xray() {
  ...
  case "$mode" in
    1)
      ...
      ;;
    2)
      read -p "请输入完整下载 URL: " download_url
      ...
      ;;
    3)
      read -p "请输入本地压缩包路径: " zip_file
      if [ -z "$zip_file" ] || [ ! -f "$zip_file" ]; then
        echo "[ERR] 本地文件不存在或路径为空"
        return 1
      fi
      ;;
    *)
  ...
  if [ "$mode" != "3" ]; then
    zip_file="/tmp/xray_update.zip"
    echo "==> 下载 Xray: $download_url"
    if ! curl -L --retry 3 --retry-delay 2 -o "$zip_file" "$download_url"; then
      echo "[ERR] 下载失败"
      return 1
    fi
  fi

  install_xray_from_zip "$zip_file" || return 1

  if systemctl is-active --quiet xray; then
    systemctl restart xray
    echo "[OK] Xray 服务已重启"
  else
    echo "[WARN] Xray 服务当前未运行，建议使用启动命令启动"
  fi
